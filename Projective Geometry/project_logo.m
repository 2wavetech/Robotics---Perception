...
% If the file contains a grayscale image, A is an M-by-N array. If the file
...
load data/BarcaReal_pts.mat        % 4x2x129 for a set of 129 clips, and 4 points on each clip 
num_ima = size(video_pts, 3);      % number of video clips
...
video_imgs = cell(num_test, 1);     % create cell array num_test by 1, one cell per clip
...
for i=1:num_test
    ...
    % Find all points, returning all (x,y) points in that image
    % within the corners, in the video frame inside the polygon defined by
    % video_pts, add these points to array interior_pts.
    % Every single pixel inside the polygon in each video frame will be
    % mapped to a point in the logo image frame.
    % note that video_pts (4x2x129)is loaded from file data/BarcaReal_pts.mat.
    ...
    warped_logo_pts = warp_pts(video_pts(:,:,test_images(i)),...   % corners on every video frame
                               logo_pts,...       % outer corners of logo image
                               interior_pts);     % all the (x, y) within goal on video frame
