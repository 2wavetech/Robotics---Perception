...
% colorImage = imread('image001.jpg');
...
corners(:,:,1) = img_pts; % corners on the 1st video frame

tracker = vision.PointTracker('MaxBidirectionalError',1);
initialize(tracker,img_pts,images{1});    % initialize point tracker

% Iterate through the rest of the images
for i = 2:size(images,1)
    %%%% CODE FOR TRACKING HERE %%%%
    % Store corners and visualize results (if desired)
    [img_pts, point_validity] = step(tracker,images{i});
    corners(:,:,i) = img_pts;
end
